...
function fetchWeather (lat, long) {
  fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&units=metric&exclude=hourly,daily&appid=${Api.key}`
  )
    ...
    .then(data => {
      ...
      document.getElementById(
        'weather'
      ).innerHTML = `${data.current.weather[0].description}`;
    });
}
/*-----------------------------------------------------------------------------
LOCATION*/
...
function showLocation (position) {
  ...
  fetch(
    `http://api.positionstack.com/v1/reverse?access_key=bf46097f2a054dfb423ac19c872c683f&query=${position.coords.latitude},${position.coords.longitude}`
  )
    .then(response => response.json())
    .then(data => {
      // console.log(data);
      document.getElementById(
        'location'
      ).innerHTML = `${data.data[0].name},<br>${data.data[0].country_code}`;
...
function getDateTime () {
  ...
  document.getElementById(
    'time'
  ).innerHTML = `${dateTime.toLocaleTimeString()}`;
}
/*-----------------------------------------------------------------------------
MENU ICON*/
